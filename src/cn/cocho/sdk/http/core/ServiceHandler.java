...
import cn.cocho.sdk.http.domain.SdkResponse;
import cn.cocho.sdk.http.utils.*;
...
public class ServiceHandler implements InvocationHandler {
    ...
    private void parseMethod(Object proxy, Method method, SdkRequest sdkRequest) {
        if (method.isAnnotationPresent(SdkMethod.class)) {
            ...
            if (StringUtilsSdk.isNotEmpty(SdkMethod.value())) {
                sdkRequest.setMethodNamespace(SdkMethod.value());
            }
            sdkRequest.setReturnType(method.getGenericReturnType());
        }

        if (StringUtilsSdk.isEmpty(sdkRequest.getMethodNamespace())) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            for (Annotation param : argAnnotations) {// 找到用SdkParam标注的参数
                if (param instanceof SdkParam) {
                    ...
                    sdkRequest.getParams().put(paramName, StringUtilsSdk.objectToString(args[i]));
                    break;
                }
            }
        }
    ...
    private void addCommonAttribute(SdkRequest sdkRequest) {
        for (Entry<String, Object> attribute : SdkContext.getCommonAttributes().entrySet()) {
            sdkRequest.getParams().put(attribute.getKey(), StringUtilsSdk.objectToString(attribute.getValue()));
        }
    }
    ...
    public Object getResultByServer(final SdkRequest sdkRequest) throws Exception {
        ...
        FutureTask<SdkResponse> futureTask = new FutureTask<SdkResponse>(new Callable<SdkResponse>() {
            @Override
            public SdkResponse call() throws Exception {
            	HttpResponse httpResponse = HttpServer.doHttpRequest(sdkRequest);
            	return formatResult(sdkRequest, httpResponse);
            }
        });
        executorService.submit(futureTask);
        SdkResponse sdkResponse = futureTask.get();
    ...
    private SdkResponse formatResult(SdkRequest sdkRequest, HttpResponse httpResponse) throws Exception {
        ...
        if (bytes == null || bytes.length == 0) {
            ...
            if (SdkContext.showLog) {
                LoggerUtilsSdk.debug("请求URL[" + sdkRequest.getRequestUrl() + "]返回的结果如下：\n" + jsonResult);
            }
...
//                sdkResponse.setResult(JsonUtilSdk.parseObject(jsonResult, sdkRequest.getReturnType()));
                sdkResponse.setResult(JsonUtilSdk.toObject(jsonResult, sdkRequest.getReturnType()));
            }
        }
        return sdkResponse;
    }
